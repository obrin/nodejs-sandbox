...
function lazyLoading () {
  ...
  const maxGroup1 = content[0].length - 1
  const maxGroup2 = content[0].length + content[1].length - 1

  return [
    content[0].slice(0, currentRow),
    content[1].slice(0, currentRow > maxGroup1 ? currentRow - maxGroup1 : 0),
    content[2].slice(0, currentRow > maxGroup2 ? currentRow - maxGroup2 : 0),
  ]
}

function lazyLoadingHorizontal (index1, index2, newIndex, currentTree, fullTree) {
  const items = fullTree[index1][index2].items.slice(0, newIndex)

  return currentTree[index1][index2].items = items
}

console.log(lazyLoadingHorizontal(0, 0, 2, content, content))
